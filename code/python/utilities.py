...
@numba.njit   
def compute_erg_dstn(tran_mat):
    
    b = np.zeros(len(tran_mat))
    b[0] = 1
   
    T = tran_mat - np.identity( len(tran_mat) )
    
    T[0,:] = b
   
    s_hat = np.linalg.solve(T, b)
    
    s = s_hat.real / np.sum(s_hat.real)
...
#@numba.njit(parallel=True)
def compose_jacobian(tran_matrices,c_pols,a_pols,init_dstn,C_ss,A_ss,c_ss,a_ss,tran_mat_ss,T):
    ...
    for i in range(T): # i is the period in which the perturbation occurs
                
        consumptionA = np.zeros(T)# List of Aggregate Consumption Values
        AssetsA = np.zeros(T) # list of Aggregate Assets values
    
        dstn = init_dstn
    
        for j in range(T):# j is the period of the aggregates we are computing
            

            if j < i+1 :#before the shock occurs

                dstn = np.dot(tran_matrices[T - 1 - i + j],dstn) #Update Distribution
                consumption = np.dot(c_pols[T- i + j],dstn) 
                #consumptionA.append(consumption)
                consumptionA[j] = consumption
                
                Assets = np.dot(a_pols[T  - i + j],dstn) 
                #AssetsA.append(Assets)
                AssetsA[j] = Assets
                
            else:# after the shock occurs simply apply the steady state values
                
                dstn = np.dot(tran_mat_ss,dstn) #Update distribution

                consumption = np.dot(c_ss,dstn) 
                #consumptionA.append(consumption)
                consumptionA[j] = consumption

                Assets = np.dot(a_ss,dstn) 
                #AssetsA.append(Assets)
                AssetsA[j] = Assets

        
        CJAC[:,i] = ((consumptionA -  C_ss)/dx)  #First differences to compute derivative
        AJAC[:,i] = ( (AssetsA - A_ss)/dx ) #First differences to compute derivative
