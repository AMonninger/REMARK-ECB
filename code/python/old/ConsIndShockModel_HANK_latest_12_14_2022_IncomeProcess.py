...
class PerfForesightConsumerType(AgentType):
    ...
    def sim_death(self):
      ...
      '''  
        ...
        DeathShks_dstn = Uniform(seed=self.RNG.randint(0, 2**31 - 1)).approx(
            N=self.AgentCount
        )
        
        DeathShks = DeathShks_dstn.draw( N=self.AgentCount)
...
init_idiosyncratic_shocks = dict(
    ...
    **{  # assets above grid parameters
        ...
        # If true Use Harmenberg 2021's Neutral Measure during simulations. For more information, see https://econ-ark.org/materials/harmenberg-aggregation?launch) 
...
class IndShockConsumerType(PerfForesightConsumerType):
        ...
        if self.cycles == 0:
            ...
            if not hasattr(dist_mGrid,'__len__'):
                ...
                if self.BoroCnstArt < 0:
                    ...
                    neg_grid = np.linspace( self.BoroCnstArt,self.aXtraMin,int(m_points/10))
    ...
    def calc_transition_matrix(self, shk_dstn = None):
        ...
        if self.cycles == 0:  # Infinite Horizon Problem
            ...
            if len(dist_pGrid) == 1: 
                ...
                self.tran_matrix = gen_tran_matrix_1D(dist_mGrid,bNext, shk_prbs,perm_shks,tran_shks,LivPrb,NewBornDist)
                
            else:
                NewBornDist = jump_to_grid_2D( np.ones_like(tran_shks), np.ones_like(tran_shks),shk_prbs,dist_mGrid,dist_pGrid )
                
                # Generate Transition Matrix
                self.tran_matrix = gen_tran_matrix_2D(dist_mGrid,dist_pGrid,bNext, shk_prbs,perm_shks,tran_shks,LivPrb,NewBornDist)
            ...
            for k in range(self.T_cycle):
                ...
                if len(dist_pGrid) == 1: 
                    ...
                    #New borns have this distribution (assumes start with no assets and permanent income=1)
                    NewBornDist = jump_to_grid_1D(np.ones_like(tran_shks),shk_prbs,dist_mGrid)
                    TranMatrix_M = gen_tran_matrix_1D(dist_mGrid,bNext, shk_prbs,perm_shks,tran_shks,LivPrb,NewBornDist)
                    ...
                    NewBornDist = jump_to_grid_2D(np.ones_like(tran_shks),np.ones_like(tran_shks),shk_prbs,dist_mGrid,dist_pGrid)
                    TranMatrix = gen_tran_matrix_2D(dist_mGrid,dist_pGrid,bNext, shk_prbs,perm_shks,tran_shks,LivPrb,NewBornDist)
    ...
    def compute_steady_state(self):
        ...
        self.A_ss =  np.dot(self.a_ss,ss_dstn)[0] # Steady State Assets
        self.C_ss =  np.dot(self.c_ss,ss_dstn)[0] # Steady State Consumption
    ...
    def calc_jacobian(self,shk_param,T):
        ...
        params['aXtraCount'] = self.aXtraCount
       ...
       # a_t.append( self.a_ss )# add steady state assets to list as it does not get appended in calc_transition_matrix method
        ...
        for i in range(T):
            da0_s.append( a_t[T    -1- i ] - a_ss )
            dc0_s.append( c_t[T   -1 - i ] - c_ss )
        ...
        dlambda0_s = []
        for i in range(T):
            dlambda0_s.append( tranmat_t[T  -1-  i ] - tranmat_ss )
        ...
        # This is step 2 of fake news algorithm
        # Expectation Vectors
        exp_vecs_a = []
        exp_vecs_c = []
        ...
        #This is step 4 of the fake news algorithm
        # Jacobian Matrices
        ...
        shock0_TM = [ tranmat_t[params['T_cycle'] - 2] ]  + (T - 1)*[self.tran_matrix] # List of transition matrices where the first transition matrix inherits the shock
        ...
        '''
        Fake News Method to obtain zeroth column
        D_ss = self.vec_erg_dstn.T[0]
        
        #np.dot(tranmat_t[-1],D_ss.T[0]) - D_ss.T[0]
        #np.dot(tranmat_t[-2],D_ss.T[0])  - D_ss.T[0]
        
        
        D_0_0 = np.dot(tranmat_t[-2],D_ss)
        ...
        for i in range(params['T_cycle'] - 1):
            
            c_first_col_without_0_0.append(np.dot(exp_vecs_c[i],D_curl_0_0))
            a_first_col_without_0_0.append(np.dot(exp_vecs_a[i],D_curl_0_0))
        
        c_first_col_without_0_0 = np.array(c_first_col_without_0_0)
        a_first_col_without_0_0 = np.array(a_first_col_without_0_0)
        
        
        A_curl_0_0 =(np.dot(a_t[-2] ,D_0_0) - np.dot(a_t[-2] ,D_ss) )/dx
        C_curl_0_0 =( np.dot(c_t[-2],D_0_0) - np.dot(c_t[-2] ,D_ss)) /dx
        
        c_first_col_0 = np.zeros(T)
        a_first_col_0 = np.zeros(T)
        
        c_first_col_0[0] = C_curl_0_0
        c_first_col_0[1:] = c_first_col_without_0_0
        
        a_first_col_0[0] = A_curl_0_0
        a_first_col_0[1:] = a_first_col_without_0_0
    ...
    def construct_lognormal_income_process_unemployment(self):
        ...
        for t in range(T_cycle):  # Iterate over all periods, counting forward
            ...
            if T_retire > 0 and t >= T_retire:
                ...
                TranShkDstn_t.X = TranShkMean_Func(taxrate[t],labor[t],wage[t])*TranShkDstn_t.X
...
class MixtureTranIncShk_HANK(DiscreteDistribution):
    ...
    def __init__(self, sigma, UnempPrb, IncUnemp, n_approx,    wage,  labor, taxrate, TranShkMean_Func, seed=0):
        ...
        dstn_approx.X = dstn_approx.X*TranShkMean_Func(taxrate,labor,wage)
