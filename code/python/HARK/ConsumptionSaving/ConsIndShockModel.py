...
class IndShockConsumerType(PerfForesightConsumerType):
    ...
    def define_distribution_grid(self, dist_mGrid=None, dist_pGrid=None, m_density = 0, num_pointsM = None,  num_pointsP = 50, max_p_fac = 7.0):
        ...
        if self.cycles == 0:
            if dist_mGrid == None:    
                aXtra_Grid = make_grid_exp_mult(
                        ming=self.aXtraMin, maxg=self.aXtraMax, ng = m_points, timestonest = 3) #Generate Market resources grid given density and number of points
        ...
        if self.cycles == 0: 
            ...
            if len(dist_pGrid) == 1: 
                ...
                for i in range(len(dist_mGrid)):
                        ...
                        TranMatrix[:,i] = LivPrb*jump_to_grid_1D(mNext_ij, shk_prbs,dist_mGrid) + (1.0-LivPrb)*NewBornDist
                ...
                for i in range(len(dist_mGrid)):
                    for j in range(len(dist_pGrid)):
                        ...
                        TranMatrix[:,i*len(dist_pGrid)+j] = LivPrb*jump_to_grid_2D(mNext_ij, pNext_ij, shk_prbs,dist_mGrid,dist_pGrid) + (1.0-LivPrb)*NewBornDist
                    '''
                #self.tran_matrix = TranMatrix
            ...
            for k in range(self.T_cycle):
                ...
                if len(dist_pGrid) == 1: 
                    ...
                    for i in range(len(dist_mGrid)):
                            mNext_ij = bNext[i]/perm_shks + tran_shks # Compute next period's market resources given todays bank balances bnext[i]
                            TranMatrix_M[:,i] = LivPrb*jump_to_grid_1D(mNext_ij, shk_prbs,dist_mGrid) + (1.0-LivPrb)*NewBornDist 
                    ...
                    for i in range(len(dist_mGrid)):
                        for j in range(len(dist_pGrid)):
                            mNext_ij = bNext[i]/perm_shks + tran_shks # Compute next period's market resources given todays bank balances bnext[i]
                            pNext_ij = dist_pGrid[j]*perm_shks # Computes next period's permanent income level by applying permanent income shock
                            TranMatrix[:,i*len(dist_pGrid)+j] = LivPrb*jump_to_grid_2D(mNext_ij, pNext_ij, shk_prbs, dist_mGrid, dist_pGrid) + (1.0-LivPrb)*NewBornDist #generate transition probabilities
    ...
    def calc_ergodic_dist(self, transition_matrix = None):
        ...
        ergodic_distr = compute_erg_dstn(self.tran_matrix[0])
