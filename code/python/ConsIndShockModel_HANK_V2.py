...
class IndShockConsumerType(PerfForesightConsumerType):
    ...
    def calc_jacobian(self,shk_param,T):
        ...
        dx = .0000001 # Size of perturbation
        ...
        ### New Code: Start
        ghost = deepcopy(FinHorizonAgent)
        ghost.solve()
        ghost.neutral_measure = True
        ghost.update_income_process()
        
        # Calculate Transition Matrices
        ghost.define_distribution_grid()        
        ghost.calc_transition_matrix()
        
        #Normalized consumption Policy Grids across time
        c_g = ghost.cPol_Grid
        a_g = ghost.aPol_Grid
        
        dstng = self.vec_erg_dstn
        C_g = []
        A_g  = []
        for i in range(params['T_cycle']):
            
            dstng = np.dot(ghost.tran_matrix[i],dstng)
            A_ghost = np.dot(a_g[i],dstng)[0]
            C_ghost = np.dot(c_g[i],dstng)[0]
            C_g.append(C_ghost)
            A_g.append(A_ghost)
...
#         a_t.append(self.a_ss)
        ...
        # New code: start
        AJAC = (A_list - A_g)/dx # Compute Impulse Response of Assets
        CJAC = (C_list - C_g)/dx # Compute Impulse Response of Consumption       
        # New code: end
       ...
       # tranmat_t.append(tranmat_ss)
        ...
        for i in range(params['T_cycle']): # i is the period in which the perturbation occurs
            ...
            for j in range(params['T_cycle']):
                
                if j < i+1 :#before the shock occurs
                    
                    dstn = np.dot(Tran_matrices[params['T_cycle'] - 1 - i + j],dstn) #Update Distribution

                    consumption = np.dot(c_t[params['T_cycle'] - 1 - i + j],dstn) 
                    consumptionA.append(consumption)
                    
                    Assets = np.dot(a_t[params['T_cycle'] - 1 - i + j],dstn) 
